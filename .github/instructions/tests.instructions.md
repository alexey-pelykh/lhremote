...
- A suite that calls `vi.stubGlobal` **must** call `vi.unstubAllGlobals()` in `afterEach`. Neither reset nor restore undoes a stub, so a stubbed `fetch` sits over the Tier-1 network guard for the remainder of the file — and that guard fails open and silently once displaced: a later test that would have been failed loudly instead receives whatever the leftover stub returns.
