...
- Tier 1's "no external dependencies" column is **enforced**, not just documented: the root `vitest.setup.ts`, wired in through `vitest.config.ts`'s `setupFiles`, replaces `fetch` and `WebSocket` for every `*.test.ts` that is not `*.integration.test.ts` or `*.e2e.test.ts`. A unit test that reaches the live network fails, and the message names the call and its site. Blocked calls are recorded as well as thrown, because the call sites that matter swallow errors (`isCdpPort()` does `catch { return false; }`), so `afterEach`/`afterAll` drain hooks fail the test even when the throw never reaches it. The escape hatch is the `*.integration.test.ts` suffix, and it is one-directional — Tier-2 work misnamed `*.test.ts` fails loudly, Tier-1 work misnamed `*.integration.test.ts` runs unguarded and silently. The guard covers `fetch` and `WebSocket` only; `node:http`, raw sockets and the `ps-list`/`pid-port` process probes stay convention-enforced. Each package carries a canary (`packages/*/src/tier1-network-guard.test.ts`) because a package-level `vite.config.*` or `vitest.config.*` would stop the root config resolving and silently detach the guard for that package alone. See ADR-004 § 2026-09-06 Amendment.
