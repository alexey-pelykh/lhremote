...
- The action-budget suite is additionally guarded against **timezone** drift, because its day boundary is the one place a test compares a JS-derived date against a SQL-derived one: `action-budget.ts` filters on `date('now','localtime')`, so the fixture must build the same *local* calendar day — `toISOString()` gives the UTC day, and the two disagree for one offset-width of every day in **both** directions, which a UTC runner cannot see and which failed nightly on any machine at a non-zero offset (#942). The `ci` job therefore re-runs that one file under `Pacific/Kiritimati` (UTC+14) and `Pacific/Midway` (UTC-11); the two are fixed-offset and their divergence windows tile every hour, so the step cannot pass vacuously. `.github/workflows/ci.yml` carries the reasoning, including why it is ubuntu-only and why it bypasses turbo.
