...
afterEach(() => {
  ...
  // last one would stay installed for the rest of the file (#935).  Bounded
  // there, not beyond it: `isolate` defaults to true, so nothing crosses into
  // the next file.  Not the Tier-1 network guard here, but the same leak.
  vi.unstubAllGlobals();
});
