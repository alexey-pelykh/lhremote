...
import { campaignRemoveAction } from "./campaign-remove-action.js";
...
describe("campaignRemoveAction", () => {
  ...
  it("returns success with campaignId and removedActionId", async () => {
    setupMocks();

    const result = await campaignRemoveAction({
      campaignId: 42,
      actionId: 7,
      cdpPort: 9222,
    });
    ...
    expect(result.removedActionId).toBe(7);
  });

  it("passes connection options to resolveAccount", async () => {
    setupMocks();

    await campaignRemoveAction({
      campaignId: 42,
      actionId: 7,
      cdpPort: 1234,
      cdpHost: "192.168.1.1",
  ...
  it("propagates resolveAccount errors", async () => {
    ...
    await expect(
      campaignRemoveAction({ campaignId: 42, actionId: 7, cdpPort: 9222 }),
  ...
  it("propagates CampaignService errors", async () => {
    ...
    vi.mocked(CampaignService).mockImplementation(function () {
      return {
        removeAction: vi.fn().mockRejectedValue(new Error("action not found")),
      } as unknown as CampaignService;
    });
