...
describe("getActionTypeCatalog", () => {
  it("returns all action types when no category is specified", () => {
    ...
    expect(catalog.actionTypes.length).toBeGreaterThanOrEqual(13);
  ...
  it("returns empty array for unknown category", () => {
    const catalog = getActionTypeCatalog("nonexistent");
    expect(catalog.actionTypes).toEqual([]);
  });
...
describe("getActionTypeInfo", () => {
  ...
  it("returns correct fields for VisitAndExtract", () => {
    ...
    expect(info!.configSchema["extractProfile"]!.type).toBe("boolean");
  });
  ...
  it("returns correct fields for MessageToPerson", () => {
    ...
    expect(info!.configSchema["messageTemplate"]!.required).toBe(true);
  ...
  it("returns correct fields for Waiter", () => {
    ...
    expect(info!.configSchema["delay"]!.required).toBe(true);
    expect(info!.configSchema["delay"]!.type).toBe("number");
  });
