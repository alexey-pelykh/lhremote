...
//   HTML comment  -- `--` collapsed and `<`/`>` stripped.  The reason stated
//                    here originally was WRONG and worth correcting rather than
//                    quietly deleting: a bare `--` does NOT terminate a comment
//                    and does NOT spill the header into the document.  Measured
//                    in Chromium -- a comment carrying two `--` sequences parses
//                    as ONE comment node, text intact, nothing leaked, markup
//                    after it untouched.  The terminator is `-->`.
//                    The collapse is still right, for the two reasons that
//                    actually hold: it neutralises a label containing the REAL
//                    terminator (`-->` collapses to `-`), and a bare `--` is
//                    non-conforming under XML/XHTML and HTML 4, so avoiding it
//                    keeps the fixture valid under tooling stricter than the
//                    HTML5 parser that actually reads it.
//                    `&` and quotes are inert inside a comment and are
//                    deliberately NOT escaped: entity-escaping there would put a
...
try {
  ...
  // No `--`: the emitted string lands inside the header HTML comment (see the
  // escaping note above), and a bare `--` there is non-conforming HTML.
