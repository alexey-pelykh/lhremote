...
network for **all tools** — a remote MCP client gains the same tool
...
- **Review every agent tool call outside the Read-only tier.** When
  using an AI agent as the MCP client, monitor its actions — every tool
  in the State-changing and Destructive tiers, and above all
  `campaign-delete`, `campaign-erase`, `delete-collection`, and
  `remove-connection`, which destroy data permanently.
